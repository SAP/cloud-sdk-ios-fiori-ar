...
internal protocol APIModel: Codable, Equatable { }

internal typealias DateTime = Date
internal typealias File = Data
internal typealias ID = UUID

internal protocol ResponseDecoder {
...
internal protocol RequestEncoder {
...
extension KeyedDecodingContainer {
    ...
    internal func decodeArray<T: Decodable>(_ key: K) throws -> [T] {
        ...
        do {
            ...
            if ARService.safeArrayDecoding {
                return array
            } else {
                throw error
            }
    ...
    internal func decodeArrayIfPresent<T: Decodable>(_ key: K) throws -> [T]? {
        return try decodeOptional {
            if contains(key) {
    ...
    fileprivate func decodeOptional<T>(_ closure: () throws -> T? ) throws -> T? {
        if ARService.safeOptionalDecoding {
            do {
                return try closure()
            } catch {
                return nil
...
internal struct DateDay: Codable, Comparable {
    ...
    internal static func == (lhs: DateDay, rhs: DateDay) -> Bool {
...
extension DateFormatter {
    ...
    internal func string(from dateDay: DateDay) -> String {
...
extension Date {
    func encode() -> Any {
        return ARService.dateEncodingFormatter.string(from: self)
    }
}
